...
function renderPlayedCards(state) {
    ...
    for (const card of state.tableCards) {
        ...
        cardElement.classList.add("table-card", `pos-${pos}`);
...
function init() {
    const savedServerUrl = localStorage.getItem("sedma.serverUrl");
    if (savedServerUrl) elements.serverUrlInput.value = savedServerUrl;

    const savedPlayerName = localStorage.getItem("sedma.playerName");
    if (savedPlayerName) elements.playerNameInput.value = savedPlayerName;
    updateActivePlayerName(savedPlayerName || elements.playerNameInput.value || "");

    const savedServerName = localStorage.getItem("sedma.serverName");
    if (savedServerName) elements.serverNameInput.value = savedServerName;
    setDeckPosition(localStorage.getItem("sedma.deckPosition") || "center");
    bindEvents();
}
init();
